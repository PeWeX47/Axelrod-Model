...
def calculate_stats(model):
    ...
    max_cluster_size, num_clusters = find_clusters(agent_features, grid.width, grid.height)
...
def bfs(agent_features, visited, target_feature, start_x, start_y, width, height):
    ...
    while queue:
        ...
        for nx, ny in neighbors:
            if not visited[ny][nx] and agent_features[ny * width + nx] == target_feature:
                queue.append((nx, ny))
                visited[ny][nx] = True
...
class AxelrodAgent(Agent):
    ...
    def step(self):
        ...
        probability = (self.model.feature_num - np.count_nonzero(self.feature - random_neighbor.feature))/self.model.feature_num
...
class AxelrodModel(Model):
    def __init__(self, N, width, height, features, traits):
        ...
        self.datacollector = DataCollector(
            model_reporters={"Smax/N": calculate_stats}
        )
    ...
    def display_plot(self):
        ...
        color_matrix = np.array(color_list).reshape(self.grid.width, self.grid.height, 3)
